...
export default function SearchFilters({ results, value, onChange, onClear }) {
  const facets = useMemo(() => {
    const conditions = countBy(results, r => r.condition);
    ...
    const unstatedCondition = results.filter(r => !r.condition).length;
    ...
    const storage = [...countBy(results, r => r.storageGb)]
      .sort((a, b) => a[0] - b[0])
      .map(([gb, count]) => ({
        value: gb,
    ...
    const colors = [...countBy(results, r => r.color)]
      .sort((a, b) => b[1] - a[1])
      .map(([c, count]) => ({ value: c, label: c, count }));

    const brands = [...countBy(results, r => r.brand)]
      .sort((a, b) => b[1] - a[1] || String(a[0]).localeCompare(String(b[0])))
      .map(([b, count]) => ({ value: b, label: b, count }));

    const sellers = [...countBy(results, r => r.source)]
      .sort((a, b) => b[1] - a[1] || String(a[0]).localeCompare(String(b[0])))
      .map(([s, count]) => ({ value: s, label: s, count }));

    const screens = [...countBy(results, r => r.screenInches)]
      .sort((a, b) => a[0] - b[0])
  ...
  const activeCount = Object.values(value).reduce((n, arr) => n + (arr?.length || 0), 0);
